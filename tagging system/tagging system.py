...
Provide the tags in snake_case format with their respective weights as a dictionary and dont add any comments."""
    ...
    try:
        ...
        content_text = content_text.strip().split("```python\n")[-1].split("\n```")[0].strip()
...
def adjust_weights_and_add_new_tags(previous_tags, new_tags,similarity_threshold=80, weight_threshold=2, max_weight=15):
    ...
    for new_tag, weight in new_tags.items():
        ...
        if matched_tag:
            updated_tags[matched_tag] = min(updated_tags.get(matched_tag, 0) + weight, max_weight)
        else:
    ...
    updated_tags = {tag: weight for tag, weight in updated_tags.items() if weight >= weight_threshold}
